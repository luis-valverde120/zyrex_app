...
exports.handleUpdateCartQuantity = exports.handleRemoveFromCart = exports.displayCart = exports.handleAddToCart = void 0;
...
const handleAddToCart = async (req, res, next) => {
    try {
        ...
        // find cart of user
        let userCart = await prisma_1.prisma.carrito.findUnique({
            where: { usuarioId: userId },
            include: { items: true }, // Include the items in the cart
        });
        if (!userCart) {
            userCart = await prisma_1.prisma.carrito.create({
                data: {
                    usuarioId: userId,
                },
        ...
        const existingItem = await prisma_1.prisma.itemCarrito.findFirst({
            where: {
        ...
        if (existingItem) {
            ...
            await prisma_1.prisma.itemCarrito.update({
                where: { id: existingItem.id },
                data: { cantidad: newQuantity },
            });
        }
        else {
            await prisma_1.prisma.itemCarrito.create({
                data: {
                    carritoId: userCart.id,
...
const handleRemoveFromCart = async (req, res, next) => {
    try {
        ...
        const deleteResult = await prisma_1.prisma.itemCarrito.deleteMany({
            where: {
                carritoId: userCart.id,
                productoId: productId,
            },
...
const handleUpdateCartQuantity = async (req, res, next) => {
    try {
        ...
        if (quantity === 0) {
            await prisma_1.prisma.itemCarrito.deleteMany({
                where: {
                    carritoId: userCart.id,
                    productoId: productId,
                },
        ...
        const productDB = await prisma_1.prisma.producto.findUnique({
            where: { id: productId },
        });
        if (!productDB) {
        ...
        const updateItem = await prisma_1.prisma.itemCarrito.updateMany({
            where: {
                carritoId: userCart.id,
                productoId: productId,
            },
...
exports.handleUpdateCartQuantity = handleUpdateCartQuantity;
